...
function addSheet(formObjectObj = {month: '2023-02'}) {
  ...
  try{
    if(formObjectObj.month === null || formObjectObj.month == ''){
       ...
       */
        let findedLastSheetWhisMonth = mainSpSheet.getSheets().filter(item => item.getName().startsWith("20"));
        ...
        if(findedSheet != null ||findedSheet != undefined){
          ...
          if(postfix >= 0){ //If there is a match, then we check if there are prefixes after the month
              ...
              addAndPlaceNewSheet(sheetName, findedLastSheetWhisMonth[findedLastSheetWhisMonth.length - 1].getIndex()+1, formObject);
              return sheetName; 
            }
        }else{
          ...
          arrSheetsToFindPlace = arrSheetsToFindPlace.filter(item => item.getName().startsWith("20"));
          findedIndex = arrSheetsToFindPlace[arrSheetsToFindPlace.length-1].getIndex();
          if(findedIndex > 0) {
            findedIndex = findedIndex;
            
          }else if(findedIndex == -1 || findedIndex === undefined){
            findedIndex = mainSpSheet.getSheetByName(template).getIndex();
          }
          sheetName = `${formObject}`;
          dataTemlate = addAndPlaceNewSheet(sheetName, findedIndex  + 1, formObject); 
        return sheetName;  
        }
    }
  }catch(e){
